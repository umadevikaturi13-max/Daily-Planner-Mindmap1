      ...
      {view === 'habits' && (
        <div style={{ paddingBottom: '80px' }}>
          <div style={st.section}>
            ...
            <div style={st.inputWrap}>
              ...
              <input 
                type="number" 
                style={{ width: '45px', border: 'none', background: 'none', fontWeight: 600 }} 
                value={hDays} 
                onChange={e => setHDays(Number(e.target.value))} 
              />
              <button style={st.addBtn} onClick={() => {
                 if(!hName) return;
                 setData({ ...data, habits: [...data.habits, { id: Date.now().toString(), name: hName, totalDays: hDays, completed: {} }] });
                 setHName("");
              }}>+</button>
            </div>
          </div>
          ...
          {data.habits.map((h: any) => (
            <div key={h.id} style={st.habitRowItem}>
              ...
              <div style={st.habitNameFixed}>
                ...
                <div style={{ fontSize: '11px', color: '#146654', fontWeight: 600 }}>
                  {Object.values(h.completed).filter(v => v).length}/{h.totalDays}
                </div>
                <button 
                  onClick={() => setData({ ...data, habits: data.habits.filter((x: any) => x.id !== h.id) })} 
                  style={st.delHabit}
                >Delete</button>
              </div>
              ...
              <div style={st.habitScrollGrid}>
                {[...Array(h.totalDays)].map((_, i) => (
                  <button 
                    key={i} 
                    onClick={() => {
                      const newH = data.habits.map((x: any) => x.id === h.id ? 
                        { ...x, completed: { ...x.completed, [i]: !x.completed[i] } } : x
                      );
        ...
        <div style={st.mindCanvas}>
          ...
          <div style={st.mindFooter}>
            <button style={st.
            } onClick={() => addMindNode()}>+ Add Root</button>
...
const st: Record<string, React.CSSProperties> = {
  ...
  habitsContainer: { paddingBottom: '80px' },
